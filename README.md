# `@battlesquid/ts-migrating` — Progressively Upgrade `tsconfig.json`

<div align="center">
  <h3>
    <a href="https://github.com/ycmjason/ts-migrating">Original by Jason Yu</a>
  </h3>
</div>
...
> The directive is named `@ts-migrating` (rather than `@ts-migration`) to reflect the plugin’s progressive and incremental philosophy.
...
1. 🔌 **[TypeScript Language Service Plugin](https://github.com/microsoft/TypeScript/wiki/Writing-a-Language-Service-Plugin)**
  ...
  * Reverts lines marked with `@ts-migrating` to be type-checked with your original `tsconfig`.
...
2. 🖥️ **Standalone CLI: `ts-migrating`**
   ...
   * `ts-migrating annotate`
    ...
    * ⚠️ Run this with a clean git state!!! This script will automatically add the `@ts-migrating` directive above every line with a TypeScript error introduced by your new `tsconfig`. Please review the changes carefully. It is recommended to run your formatter and linter afterwards. You may need to run this command again after formatting or linting.
...
This project does **NOT** require any IDE extensions. It relies purely on TypeScript's own Language Service, so it works on most IDEs and editors that support TypeScript (e.g., VS Code, WebStorm).

To install:

```bash
cd my-cool-project
npm install -D ts-migrating@npm:@battlesquid/ts-migrating
```

The npm alias keeps the local package name as `ts-migrating`, which is what TypeScript uses to resolve the plugin and what the examples below import from. The published package is `@battlesquid/ts-migrating`.
...
The functions are exposed via `ts-migrating/api` when installed with the alias shown above:
